...
char escape_char(const char c, int line, int column)
{
	if (c == 't')
	{
		return '\t';
	...
	else
	{
		...
		Error(working_dir + current_file_name + ":" +to_string(line + 1) + ":" + to_string(column), err);
	}
	return ' ';
}
	...
	while (*(src + index) != '\0')
	{
		...
		{ // Les commentaires avec '//'
			if (word != "") // On peut garder ce qu'il y a avant
			{
				...
				ref.push_back(to_string(line+1) + ":" + to_string(column));
				word = "";
			}
		...
		else if (*(src + index) == '"')
		{
			...
			while (*(src + secon_index + index) != *(src + index))
			{
				...
				{
					Error(working_dir + current_file_name + ":" +to_string(line+1) + ":" + to_string(column), "Unmatched '\"' token");
					break;
				}
				else if (*(src + secon_index + index) == '\\')
				{
					word += escape_char(*(src + secon_index + index + 1), line, column);
					secon_index++;
				}
				else
				{
		...
		else
		{
			...
			error_mess += "'";
			Error(working_dir + current_file_name + ":" + to_string(line + 1) + ":" + to_string(column), error_mess);
		}
		index++;
	}
