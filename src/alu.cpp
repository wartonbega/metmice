...
void error_generator_alu(string ope, Mtmc_variable *one, Mtmc_variable *two, string type1, string type2)
{
    ...
    {
        full += to_string(*(int64_t *)two->content);
    }
    else if (type2 == "list")
    {
        full += "list";
    }
    else if (is_typed_non_predefined(type2))
    {
        Mtmc_class_variable* arg2_prime = (Mtmc_class_variable *)two->content;
        //full += arg2_prime->convert_to_string(vector<string>());
        full += "...";
    }
    else
    {
