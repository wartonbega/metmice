...
Les fonctions :
	*()[]{};
	Attention pas d'assignations possibles ! (pas de ->)
...
Node *give_if(Node *trunc, int index)
{
	...
	current_node->ref = trunc->children[index]->ref;
	push_child(current_node, trunc->children[index]); // if
	...
	if (trunc->children[index + 3]->value == "else") 
	{
		...
		push_child(current_node, trunc->children[index + 4]); // {}
	}
	return current_node;
}
...
Node *give_while(Node *trunc, int index)
{
	...
	current_node->ref = trunc->children[index]->ref;
	push_child(current_node, trunc->children[index]); // while
	push_child(current_node, trunc->children[index + 1]); // ()
	push_child(current_node, trunc->children[index + 2]); // {}
	return current_node;
}
...
Node *give_for(Node *trunc, int index)
{
	...
	current_node->ref = trunc->children[index]->ref;
	push_child(current_node, trunc->children[index]); // for
	push_child(current_node, trunc->children[index + 1]); // ()
	push_child(current_node, trunc->children[index + 2]); // {}
	return current_node;
}
...
Node *give_expr(Node *trunc, int index)
{
	...
	*/
	...
	{
		push_child(current_node, trunc->children[index + 1]); // "->"
		push_child(current_node, trunc->children[index + 2]); // "*s"
	}
	return current_node;
}
...
Node *give_keyword(Node *trunc, int index)
{
	...
	current_node->ref = trunc->children[index]->ref;
	push_child(current_node, trunc->children[index]); // keyword (exemple : return, push ...)
	if (trunc->children[index + 1]->value == "*")
	{
		push_child(current_node, trunc->children[index + 1]); // *
	}
...
vector<Node *> recognize_paternes(Node *trunc, bool debug)
{
	...
	vector<string> reference = childs_ref(trunc);
	Node *last = new Node();
	int index = 0;
	while (index < c_values.size())
	{
		if (debug)
		{
			if (recognize_struct_debug(c_values, index, reference))
			{
				last = give_struct(trunc, index);
			}
			else if (recognize_if_debug(c_values, index, reference))
			{
				last = give_if(trunc, index);
			}
			else if (recognize_func_debug(c_values, index, reference))
			{
				last = give_func(trunc, index);
			}
			else if (recognize_while_debug(c_values, index, reference))
			{
				last = give_while(trunc, index);
			}
			else if (recognize_for_debug(c_values, index, reference))
			{
				last = give_for(trunc, index);
			}
			else if (recognize_keyword_debug(c_values, index, reference))
			{
				last = give_keyword(trunc, index);
			}
			else if (recognize_expr_debug(c_values, index, reference))
			{
				last = give_expr(trunc, index);
			}
			else
			{
				string err = "unknown instruction `" + get_from(c_values, index, int(c_values.size())) + "` \n(Warning : this instruction can be shorten)";
				Error("paterns", err);
			}
		}
		else
