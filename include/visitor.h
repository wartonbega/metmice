...
void compute_operator_normal(vector<string> names, vector<string> types, int &i, bool funcall, map<string, Mtmc_variable *> &variable_function, Node *trunc, int recursive);
string while_loop(vector<Node *> ast, bool function, map<string, Mtmc_variable *> &variable_function, int recursive, vector<Node *> args);
    ...
    while (index != ast.size())
    {
        ...
        else if (instruction->value == "if_statement")
        {
            ...
            if (cond == "true")
            {
                ...
                normal_code_execution(paterns, false, prout, 0, false); // Qui correspond au {}
            }
            else if (instruction->children.size() > 3 and instruction->children[3]->value == "else")
            {
                ...
                normal_code_execution(paterns, false, prout, 0, false); // Qui correspond au {} du else statement
            }
        }
