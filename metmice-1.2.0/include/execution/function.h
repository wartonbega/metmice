    ...
    while (index != ast.size())
    {
        ...
        else if (instruction->value == "if_statement")
        {
            ...
            if (*(string *)condition_value->content == "true")
            {
                ...
                string ret = normal_code_execution(paterns, true, variable_function, recursive); // Qui correspond au {}
                if (ret == "return")
                {
                    what_to_return = computes.top();
                    computes.pop();
            ...
            else if (instruction->children.size() > 3 and instruction->children[3]->value == "else")
            {
                ...
                string ret = normal_code_execution(paterns, true, variable_function, 0); // Qui correspond au {} du else statementif (ret == "return")
                if (ret == "return")
                {
                    if (computes.size() >= 1)
                    {
