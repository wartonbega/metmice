...
section_loop_begin: // On utilise les sections et les goto pour plus de praticité
    ...
    if (i < end_as_int)
    {
        ...
        string ret = normal_code_execution(ast, function, variable_function, recursive, true);
        if (ret == "return")
        {
            what_to_return = ret;
...
Mtmc_variable *while_loop_compute_condition(bool function, map<string, Mtmc_variable *> &variable_function, int recursive, vector<Node *> args)
{
