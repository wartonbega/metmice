...
vector<Mtmc_variable *> compute_listed_values_function(vector<Node *> listed_values, map<string, Mtmc_variable *> &variable_function, int recursive);
...
Mtmc_variable *compute_function(Node *trunc, bool empty_stack, bool funcall, map<string, Mtmc_variable *> &variable_function, int recursive)
{
    ...
    while (index != ast.size())
    {
        ...
        else if (instruction->value == "keyword")
        {
            ...
            if (instruction->children[0]->value == "push")
            {
                ...
                int cargs1 = *(int *)arg1_push->content;
                if (cargs1 == 1)
                {
                    empty_compute_stack();
                    push_display_pile(arg2_push);
                }
                else if (cargs1 == 2)
                {
                    empty_compute_stack();
                    computes.push(arg2_push);
                }
                else if (cargs1 == 3)
                {
                    if (!Mtmc_variable_type_check(arg2_push, "int"))
                    {
                        Error("execution", "arguments for 'push' in the system pile needs to be an integer");
                    }
                    system_p.push_back(*(int *)arg2_push->content);
                }
            }
